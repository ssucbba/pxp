...
class MODSigepServiceRequest extends MODbase{
	...
	function procesarSigep ($link, $accessToken, $status, $url, $method, $id_sigep_service_request, $id_service_request, $sigep_container = '') {
		...
		var_dump($params);exit; //ver parametros enviados o antes de ser enviados
		$curl = curl_init();
		...
		if ($method != 'GET') {
		    //aca se puede hacer var_dump de los parametros enviados
	    ...
	    if ($err) {	    	
	        ...
	        try {
				...
				var_dump('salida de proceso:', $resObj);exit;
				//aqui podemos hacer var dump de la respuesta
				if (isset($resObj['data']['errores'])) {
					...
					$this->registrarProcesoExitoso($link,$id_sigep_service_request,($sigep_container == '' ? $resObj['data']:$resObj['C31'][$sigep_container]),$id_service_request);
				}		
